...
module Machine.Test.SumArray where
...
--------------------------------------------------------------------------------
prog :: Program
prog = unsafePerformIO $ readProgram "examples/sumArray.asm"

addWorks :: IO ()
addWorks = do
    print "Sum Array works as 'Prelude.sum'."
    let steps = 3
        machineAdd x y = let mem = initialiseMemory [ (0, x)
                                                    , (1, y)
                                                    , ]
