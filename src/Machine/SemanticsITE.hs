...
-- -- | Functorial semanticsITE is data independent and may have a most one
-- --   static dependency. May be used for
-- --   static analysis.
-- semanticsF :: MachineValue a => Instruction
--                              -> SemanticsITE Functor MachineKey a ()
-- semanticsF = \case
--     Halt           -> haltF
...
-- -- | Applicative semanticsITE is data independent. May be used for
-- --   static code analysis.
-- --
-- --   Note: applicative semanticsITE cannot interact with flags.
-- semanticsA :: MachineValue a => Instruction
--                              -> SemanticsITE Applicative MachineKey a ()
...
--                              -> SemanticsITE Selective MachineKey a ()
...
-- -- | Monadic semanticsITE may involve data dynamic analysis and must be executed
...
--                              -> SemanticsITE Monad MachineKey a ()
...
-- | Halt the execution.
--   Functor.
haltF :: MachineValue a => SemanticsITE Functor MachineKey a ()
haltF read write _ = Just $
    write (F Halted) ((const 1) <$> read (F Halted))

-- | Halt the execution.
--   Applicative.
haltA :: MachineValue a => SemanticsITE Applicative MachineKey a ()
...
--   Selective.
addS :: MachineValue a => Register -> MemoryAddress -> SemanticsITE Selective MachineKey a ()
...
abs :: MachineValue a => Register -> SemanticsITE Functor MachineKey a ()
...
-- | Unconditional jump.
--   Functor.
jump :: MachineValue a => SImm8 -> SemanticsITE Functor MachineKey a ()
...
--   Selective.
jumpZero :: MachineValue a => SImm8 -> SemanticsITE Selective MachineKey a ()
jumpZero simm read write ite = Just $
    ite (eq <$> read (F Zero) <*> pure 0)
        (write IC (fmap ((+) . unsafeFromSImm8 $ simm) (read IC)))
        (write IC $ read IC)
--------------------------------------------------------------------------------
-- executeInstruction :: SemanticsITE Monad MachineKey Value ()
-- executeInstruction read write _ = Just $ do
--     -- fetch instruction
--     ic <- read IC
