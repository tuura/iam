...
module Machine.Semantics.Symbolic.SymEngineTyped where
...
import Machine.SemanticsTyped hiding (jumpZero)
import Machine.Semantics.Symbolic.TypesTyped
...
whenSym :: SymEngine (Sym Bool) -> SymEngine (Sym ()) -> SymEngine (Sym ())
whenSym x y = -- select (bool (Right ()) (Left ()) <$> x) (const <$> y)
    SymEngine $ \s ->
    concat [f a (snd b) | a <- runSymEngine x s, b <- runSymEngine y (snd a)]

f :: (Sym Bool, SymState) -> SymState -> [(Sym (), SymState)]
f (b, sNoExec) sOnExec =
    [ (sConst (), appendConstraints [b] sOnExec)
    , (sConst (), appendConstraints [(sNot b)] sNoExec)]
...
jumpZero simm = void $
    let inc (SConst x) = SConst (x + unsafeFromSImm8 simm)
    in whenSym (readKey (F Zero))
               (sConst <$> writeKey IC (inc <$> (readKey IC)))
...
readKey :: MachineKey a -> SymEngine (Sym a)
readKey = \case
...
symStep state =
    ...
    in -- (snd . ((flip runSymEngine) fetched)) <$>
       case i of
          JumpZero offset -> map snd $ runSymEngine (jumpZero offset) fetched
          _ -> map snd $ runSymEngine (semanticsM i readKey undefined) fetched
...
writeMemory :: MemoryAddress -> Sym Val -> SymEngine ()
...
readMemory :: MemoryAddress -> SymEngine (Sym Val)
...
readRegister :: Register -> SymEngine (Sym Val)
...
writeRegister :: Register -> (Sym Val) -> SymEngine ()
...
zeroNotSet :: SymState -> Sym Bool
zeroNotSet s = (Map.!) (flags s) Zero
    -- (sEq ((Map.!) (flags s) Zero) (SConst False))
