...
sWordEx i name =  ask >>= liftIO . SBV.svMkSymVar (Just SBV.EX) (SBV.KBounded False i) (Just name)
...
symToSMT _ (SConst w) =  return $ wordToSVal w
symToSMT m (SNot c) =
  let c' = symToSMT m c
...
--   in
--   sValToSWord <$> (SBV.svOr <$> l' <*> r')
symToSMT m (SAnd l r) =
  let l' = sValToSBool <$> symToSMT m l
      r' = sValToSBool <$> symToSMT m r
  in
    sValToSWord <$> (SBV.svAnd <$> l' <*> r')
-- symToSMT m (SLt l r) =
...
wordToSVal :: Word -> SBV.SVal
wordToSVal w = SBV.svInteger (SBV.KBounded False 32) (toInteger w)

sValToSBool :: SBV.SVal -> SBV.SVal
sValToSBool w = w `SBV.svNotEqual` (wordToSVal 0)

sValToSWord :: SBV.SVal -> SBV.SVal
sValToSWord w = SBV.svIte w (wordToSVal 1) (wordToSVal 0)
...
renderSolvedState (SolvedState state c) =
  "PC: " <> show (instructionCounter state) <> "\n" <>
--   "Stack: " <> show (renderSym <$> st) <> "\n" <>
  "Path Constraints: " <> show (show (foldr SAnd (SConst 1) (pathConstraintList state))) <> "\n" <>
...
solveSym (Tree.Node state c) = do
  ...
  SBV.SatResult smtRes <- SBV.satWith SBV.z3 smtExpr
  children <- traverse solveSym c
  return $ Tree.Node (SolvedState state smtRes) children
